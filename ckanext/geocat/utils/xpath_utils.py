...
SERVICE_PROTOCOLS = [
    protocol for protocol in NORMED_PROTOCOLS if protocol != DOWNLOAD_PROTOCOL
]
...
def xpath_get_language_dict_from_geocat_multilanguage_node(node):
    ...
    value = node.xpath(".//gco:CharacterString/text()", namespaces=gmd_namespaces)
    if value:
...
def xpath_get_url_and_languages_for_data_model(node):
    data_model_node = xpath_get_all_sub_nodes_for_node_and_path(
    ...
    if data_model_node:
        ...
        urls_list, _ = xpath_get_values_list_from_path_list(
            node=node, path_list=CONFORMS_TO_URL_PATH_LIST
        )
        for locale in LOCALES:
            value_locale = node.xpath(
                './/che:LocalisedURL[@locale="#{}"]'.format(locale) + "/text()",
...
def xpath_get_url_with_label(node):
    ...
    value_locale_en = node.xpath(
        './/gmd:LocalisedCharacterString[@locale = "#EN"]/text()',
        namespaces=gmd_namespaces,
    )
    ...
    text_node = node.xpath(GMD_URL_LABEL, namespaces=gmd_namespaces)
    if text_node:
        url_text_node = xpath_get_one_value_from_geocat_multilanguage_node(text_node[0])
        if url_text_node:
...
def xpath_get_url_from_node(node):
    url_node = node.xpath(".//gmd:linkage/gmd:URL/text()", namespaces=gmd_namespaces)
    if url_node:
        return url_node[0]
    for locale in LOCALES:
        url_node = node.xpath(
        ...
        if url_node:
            return url_node[0]
    url_node = node.xpath(".//che:LocalisedURL/text()", namespaces=gmd_namespaces)
    if url_node:
        return url_node[0]
    return None
...
def xpath_get_distribution_from_distribution_node(resource_node, protocol):
    ...
    if name_node:
        distribution["name"] = xpath_get_language_dict_from_geocat_multilanguage_node(
            name_node
        )
    else:
    ...
    if description_node is not None:
        distribution[
            "description"
        ] = xpath_get_language_dict_from_geocat_multilanguage_node(description_node)
    else:
    ...
    _set_distribution_format_and_media_type(protocol, normed_protocol, distribution)

    GMD_URL = ".//gmd:linkage"
    url_node = xpath_get_single_sub_node_for_node_and_path(
        node=resource_node, path=GMD_URL
    )
    if url_node is not None:
        distribution_url, distribution_language = xpath_get_url_and_languages(url_node)
...
def _set_distribution_format_and_media_type(
    ...
    protocol_is_download_protocol_and_has_format = (
        normed_protocol == DOWNLOAD_PROTOCOL
        and protocol.startswith(DOWNLOAD_PROTOCOL + ":")
    )
...
def xpath_get_geocat_services(node):
    service_nodes = xpath_get_all_sub_nodes_for_node_and_path(
    ...
    if service_nodes:
        for service_node in service_nodes:
            ...
            geocat_service["url"], _ = xpath_get_first_of_values_from_path_list(
...
def _is_valid_url(url):
    ...
    try:
        ...
        log.warning("provided URL {} for conforms_to field is not valid".format(url))
        return ""
